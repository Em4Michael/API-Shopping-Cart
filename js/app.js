	...
	fetch('https://fakestoreapi.com/products')
			...
			//console.log(data);
				...
				completedata.map((values)=>{
					...
					<div class="pp">
					<div class="col-auto">
						<div class="card-f" style="width: 200px;">
							<a href="product.html">
...
<td><a href="#" class="remove">Remove</a></td>`
	...
	const y = JSON.stringify(LocalItem); */

	
	/* const CartItem = localStorage.getItem('keys')
	let cartitem = JSON.parse(CartItem)
	if (Cartitem === null)
			product.push(LocalItem)
			localStorage.setItem('keys', JSON.stringify(LocalItem))
		}  */
		...
		//const put = localStorage.getItem('id');
...
/*
	class	localCart{
		...
		static getlocalCartItems(){
			...
			if (CartItem === null || CartItem.length ===0) return cartMap
			new map(Object.entries(JSON.parse(CartItem)))
		}
		static addItemTolocalCart(id, item){}
		static removeItemTolocalCart(id){}


	}
	
	if (title && price && imgSrc){
		localStorage.setItem(title, price, imgSrc);
		const put = localStorage.getItem(price);
		//location.reload();
		console.log(put)
	} */
}
...
function CartTotal() {
	...
	document.getElementsByClassName('cartt')[0].innerText = 
	document.getElementsByClassName('carttt')[0].innerText = total
	document.getElementsByClassName('see')[0].innerText = kk
